...
// CheckOwnership enforces the owner-or-elevated policy shared by
// `quest update`, `complete`, and `fail`. Per spec §quest accept,
// after acceptance only the owning session (or an elevated role) can
// mutate the task. Empty ownerSession (the direct-close-by-lead case)
// fails for non-elevated callers because the semantics of "anyone
// can close a task nobody owns" would leak worker-level access into

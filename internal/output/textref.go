...
// reference in --format text output: the show header, the parent
// metadata row, show dependency rows, and graph node / edge lines.
// Routing every call site through one formatter keeps the `(bug)`
// marker consistent when the spec evolves (e.g. a new marker for
// another type).
